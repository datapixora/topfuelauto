...
62. [x] Data Engine STEP 7: Auto-Merge Rules - added should_auto_merge() validation function to data_engine_service.py with 4-rule validation (source auto_merge_enabled flag, required fields check, duplicate prevention via canonical_url, quality filters for price/year), added auto_merge_listing() function to execute merge with attributes copy, integrated auto-merge into scraper workflow (_execute_scrape checks rules after staging and auto-merges valid listings), added auto_merge_enabled checkbox toggle to source creation form UI, tracks auto-merged count and manual review reasons in run debug_info. Also improved proxy settings form to use separate host/port fields (Proxy Server + Port) instead of combined URL field for better UX.
