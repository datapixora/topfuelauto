...
101. [x] FIX CASCADE DELETE: Fixed DELETE /admin/data/sources/{id} returning 500 IntegrityError. Root cause: FK constraints missing ON DELETE CASCADE in production database. Created migration 0023_fix_cascade (revised from 0023_fix_cascade_delete_constraints to fit 32-char limit) using DROP CONSTRAINT IF EXISTS + op.create_foreign_key with ondelete="CASCADE" for three FK chains (admin_runs.source_id → admin_sources.id, staged_listings.run_id → admin_runs.id, staged_listing_attributes.staged_listing_id → staged_listings.id). Migration is idempotent and production-safe. Updated router to catch IntegrityError and return 409 with helpful message instead of 500. Added comprehensive regression tests (6 test cases) covering cascade delete chains. Proper status codes: 204 on success, 404 if not found, 409 if constraint violation (should not happen after migration).
