...
117. [x] Bidfax proxy wiring implemented: `auction_tracking` now stores proxy_id/exit_ip/error (migration 0030) and model updated. Bidfax jobs/test-parse accept proxy_id, validate via proxy_service, build proxy_url, and run fetches through proxy with health check + actionable 403 hints. Celery worker fetches via proxy (records proxy diagnostics/backoff on failure). Bidfax provider supports proxy_url. Admin UI (/admin/data/sold-results) adds proxy dropdowns for Create Job + Test Parse, passes proxy_id, and displays proxy/exit IP/error in tracking table. API client/types updated accordingly.

118. [x] BROWSER FETCH MODE (Playwright): Implemented parallel browser-based fetcher alongside HTTP for Bidfax sold results scraping. Architecture: Created normalized `FetchDiagnostics` dataclass (html, status_code, latency_ms, fetch_mode, final_url, error, proxy_exit_ip, browser_version) returned by both HTTP and browser fetchers. Created `BrowserFetcher` class using Playwright sync_api Chromium (headless mode, anti-detection args `--disable-blink-features=AutomationControlled`, proxy support via parsed config dict, exit IP detection via ipify.org navigation, timeout 30s). Refactored `HttpFetcher` from `BidfaxHtmlProvider` to match interface (rate limiting 30 req/min, realistic headers preserved). Updated `BidfaxHtmlProvider.fetch_list_page()` to accept `fetch_mode` parameter ("http" or "browser") and return `FetchDiagnostics` instead of raw HTML string. Schemas: Added `fetch_mode: str = Field(default="http")` to `BidfaxJobCreate`, `BidfaxTestParseRequest`, and `DebugInfo` (backward compatible default). API: Updated test-parse endpoint to use `fetch_result.html` and capture diagnostics (status_code, latency_ms, proxy_exit_ip from FetchDiagnostics), pass `fetch_mode` to provider, include fetch_mode in debug section. Updated create job endpoint to pass fetch_mode to Celery task. Worker: Modified `enqueue_bidfax_crawl` to accept fetch_mode parameter and store in `tracking.stats["fetch_mode"]` (JSONB, no schema changes), updated `fetch_and_parse_tracking` to read fetch_mode from tracking.stats (default "http"), use fetch_result from FetchDiagnostics, preserve fetch_mode in stats on success. Testing: Added 11 comprehensive unit tests in `test_bidfax_provider.py` covering: BrowserFetcher initialization, proxy URL parsing (http://user:pass@host:port → Playwright dict), successful fetch with mocked Playwright, proxy configuration capture, navigation error handling, HttpFetcher interface validation, BidfaxProvider fetch_mode selection, FetchDiagnostics structure, schema defaults, fetch_mode="browser" option. Files created: `fetch_diagnostics.py` (dataclass), `fetchers/browser_fetcher.py` (Playwright implementation), `fetchers/http_fetcher.py` (extracted HTTP logic), `fetchers/__init__.py` (exports). Files modified: `providers/bidfax.py` (uses fetchers), `schemas/auction.py` (fetch_mode fields), `routers/admin_auction.py` (test-parse + create job), `workers/auction.py` (enqueue + worker tasks). Features: parallel fetcher (not replacement), normalized results, existing parser unchanged, no DB schema changes, proxy pool compatible, backward compatible (defaults to HTTP mode). Use case: enables scraping JavaScript-heavy auction pages when HTTP fetch returns empty/blocked responses.
