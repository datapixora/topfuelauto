...
101. [x] FIX CASCADE DELETE: Fixed DELETE /admin/data/sources/{id} returning 500 IntegrityError. Root cause: FK constraints missing ON DELETE CASCADE in production database. Created migration 0023 to drop and recreate FK constraints with explicit CASCADE (admin_runs.source_id → admin_sources.id, staged_listings.run_id → admin_runs.id, staged_listing_attributes.staged_listing_id → staged_listings.id). Migration is idempotent and checks for existing constraints before dropping. Updated router to catch IntegrityError and return 409 with helpful message instead of 500. Added comprehensive regression tests (6 test cases) covering cascade delete chains with runs, listings, and attributes. Proper status codes: 204 on success, 404 if not found, 409 if constraint violation (should not happen after migration).
